...
typedef enum pd_bits
{
    ...
    kPDRUNCFG_PD_LPO_OSC = SYSCON_PDRUNCFG_LPOSC_PD_MASK,
...
enum _power_wakeup
{
    ...
    kPDAWAKECFG_Wakeup_LP_OSC  = SYSCON_PDAWAKECFG_LPOSC_PD_MASK,
...
static inline void POWER_EnableLPO(bool enable)
{
    if (enable)
    {
        PMU->DPDCTRL |= PMU_DPDCTRL_ULPOSCEN_MASK;
    }
    else
    {
        PMU->DPDCTRL &= ~PMU_DPDCTRL_ULPOSCEN_MASK;
...
static inline void POWER_EnableLPOInDeepPowerDownMode(bool enable)
{
    if (enable)
    {
        PMU->DPDCTRL |= PMU_DPDCTRL_ULPOSCDPDEN_MASK;
    }
    else
    {
        PMU->DPDCTRL &= ~PMU_DPDCTRL_ULPOSCDPDEN_MASK;
